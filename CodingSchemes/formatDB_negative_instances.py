...
def filterDBLineages(file,schema,maxlength):
	...
	for te in SeqIO.parse(file, "fasta"):
		...
		if len(te.seq) <= maxlength:
			...
			# looking for label
			order = -1
			if str(te.id).upper().find("POSITIVA") != -1:
				order = 1
			elif str(te.id).upper().find("NEGATIVA") != -1:
				order = 2
			print(order)
			if order != -1:
...
def kmerDB(file, id, seqs_per_procs, kmers, TEids, TEseqs, n, remain):
	...
	while init < end and init < n:
		order = -1	
		if str(TEids[init]).upper().find("POSITIVA") != -1:
			order = 1
		elif str(TEids[init]).upper().find("NEGATIVA") != -1:
			order = 2
		if order != -1:
...
if __name__ == '__main__':
	# 
	file = "/data3/projects/HERV/Candamil/Procesos_II/positive_database/esquemas_codificacion/base_datos_random.fasta"
