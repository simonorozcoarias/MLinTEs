...
def classification(filename, prepro, threads):
    ...
    models = []
    #schemes = ["EIIP","DAX", "enthalpy"]# este es para SVM que falta
    #schemes = ["enthalpy"]
    #schemes = ["kmers"]#, "pc", "complementary", "DAX", "EIIP", "enthalpy", "Galois4", "orthogonal"]
    schemes = ["pc", "complementary", "DAX", "EIIP", "enthalpy", "Galois4", "orthogonal"]
...
'''
        ...
        svc = OneVsRestClassifier(SVC(C=x[yValidation.index(max(yValidation))],gamma=1e-6), average='macro')
        svc.fit(X_train, Y_train)
