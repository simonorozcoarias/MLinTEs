...
# ### Quinto Tuneo: Support Vector Machine

# In[89]:


estimator = SVC(cache_size=memory,class_weight='balanced')
text_model = "SVC"
...
parameters = {'kernel':['linear','poly','sigmoid','rbf'],
               ...
               'gamma':[1*(10**i)for i in range(-4,2,1)],
               'tol':[1*(10**i)for i in range(-7,1,1)],
               'C':[1*(10**i)for i in range(-4,5,1)],
               }
...
start_t = time.time()
tunned_clf = GridSearchCV(estimator,parameters, scoring=scoring, cv=CV, n_jobs=n_jobs, verbose=3)
...
tunnedModel = OneVsRestClassifier(SVC(**finalParams),n_jobs=n_jobs)
