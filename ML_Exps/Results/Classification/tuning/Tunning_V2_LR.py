...
results = []
names = []
filename = "/shared/home/rguyot/coffea_genomes/Simon/Tunning/InpactorDB_v1.fasta.kmers"
n_jobs = 52
...
# ### Primer Tuneo: Logistic Regression
...
parameters = {'C':[1*(10**i)for i in range(-4,5,1)],
      'tol':[1*(10**i)for i in range(-4,5,1)],
      'max_iter':[1*(10**i)for i in range(0,6,1)],
...
hype = 'penalty'
parameters2 = dict()
if tunned_clf.best_params_[hype] == 'l1':
    parameters2['solver']=['saga', 'liblinear']
elif tunned_clf.best_params_[hype] == 'l2':
    parameters2['solver']=['newton-cg', 'lbfgs', 'sag','saga']
elif tunned_clf.best_params_[hype] == 'elasticnet': 
    parameters2['solver']=['saga']
elif tunned_clf.best_params_[hype] == 'none':
    parameters2['solver']=['newton-cg', 'lbfgs','sag', 'saga']
...
tunnedModel = LogisticRegression(**finalParams)
