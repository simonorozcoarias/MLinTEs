...
def sequences_extractor(splitted_genome, position_predictions, x, seqs_per_procs, n, remain, outputdir,
    ...
    extracted_seqs = np.zeros((1, 1), dtype=bool)
    if i < m:
        predicted_ids = []
            ...
            if isTE > detec_threshold:
                ...
                if len(bestCandidates) > 0:
                    extracted_seqs_i = np.zeros(
                        (len(bestCandidates), splitted_genome.shape[1], splitted_genome.shape[2]), dtype=bool)
                    ...
                    for c in range(len(bestCandidates)):
                        ...
                        predicted_ids.append(
                            seq_id + "#" + str(init + factor) + "#" + str(end + factor) + "#" + str(isTE))
...
"""
def Inpactor2_Filter(kmer_counts, ids_predicted, predicted_ltr_rts, filter_threshold):
    installation_path = os.path.dirname(os.path.realpath(__file__))
    # Scaling
    ...
    filtered_elements = np.zeros((len([x for x in binary_predictions if x == 0]), kmer_counts.shape[1]), dtype=bool)
    new_ltr_predicted = np.zeros(
        (len([x for x in binary_predictions if x == 0]), predicted_ltr_rts.shape[1], predicted_ltr_rts.shape[2]),
        dtype=bool)
    new_ids_predicted = []
...
def non_maximal_suppression(ids_predicted, predictions, percentages, iou_threshold, curation, predicted_ltr_rts):
    ...
    new_ltr_predicted = np.delete(predicted_ltr_rts, deleted_seqs, 0)

    return finalIds, new_predictions, new_percentages, new_ltr_predicted


def non_maximal_suppression_viejo(ids_predicted, predictions, percentages, iou_threshold, curation, predicted_ltr_rts):
    finalIds = []
    elements_dict = {}
    ...
    for seqid in finalIds:
        ...
        if curation:
            percFilt = float(columns[4])
            elements_dict[seqid] = (percDect + percFilt + perClass) / 3
        else:
            elements_dict[seqid] = (percDect + perClass) / 2
    ...
    for key in sorted_dict.keys():
        if key not in marked_seqid:
            ...
            # to delete repeat predictions
            repeated_seqs = [x for x in range(len(finalIds)) if finalIds[x] == key]
            if len(repeated_seqs) > 1:
                deleted_pos.extend(repeated_seqs[1:])

            percentage = sorted_dict[key]
            for seqid in sorted_dict.keys():
                if key != seqid:
                    pos_seqId = finalIds.index(seqid)
                    if iou(key, seqid) > iou_threshold and percentage >= sorted_dict[seqid]:
...
if __name__ == '__main__':
    ...
    predictions_cycles = []
    percentages_cycles = []
    ltr_predicted_final_cycles = np.zeros((10, 10), dtype=bool)
    for cycle in range(0, cycles):
        ...
        localresults = [pool.apply_async(sequences_extractor,
                                         args=[splitted_genome_list[x], position_predictions, x, seqs_per_procs, n,
                                               remain,
                                               outputDir, max_len_threshold, min_len_threshold, list_ids, tg_ca, TSD,
                                               detec_threshold]) for x
                        in range(threads)]
        localTables = [p.get() for p in localresults]
        ...
        if curation:
            ...
            start = time.time()
            filtered_seqs, ids_predicted, new_ltr_predicted = Inpactor2_Filter(kmer_counts, ids_predicted,
                                                                               ltr_rts_predicted, filter_threshold)
        ...
        if ltr_predicted_final_cycles.shape[1] == 10:  # first creation of the np.array
            ltr_predicted_final_cycles = new_ltr_predicted
        else:
            ltr_predicted_final_cycles = np.concatenate((ltr_predicted_final_cycles, new_ltr_predicted), axis=0)
