...
"""
def kmer_counting(file, id, seqs_per_procs, kmers, TEids, n, remain, outputDir):
	if id < remain:
	...
	while init < end and init < n:
		...
		posSeq = int(TEids[init].split("#")[0])
		realId = ids_from_seqs[posSeq]
		#####################

		# to extract the nucleotide sequence form fasta file
		initSeq = int(TEids[init].split("#")[1])
		endSeq = int(TEids[init].split("#")[2])
		TEseq = [seq.seq for seq in SeqIO.parse(outputDir+'/'+file, "fasta") if seq.id == realId][0]
		if (endSeq + initSeq) > len(TEseq):
			endSeq = len(TEseq)
		TEseq = TEseq[initSeq: endSeq]
		#####################
...
if __name__ == '__main__':
	...
	start = time.time()	
	TEids = splitting_fasta_file(file, length, window, outputDir)
	finish = time.time()
	...
	if kmer_file == None:
		...
		remain = n % threads
		processes = [multiprocessing.Process(target=kmer_counting, args=[file, x, seqs_per_procs, kmers, TEids, n, remain, outputDir]) for x in range(threads)]
