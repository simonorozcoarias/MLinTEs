...
def sequences_extractor_master(splitted_genome_ltr, threads, outputDir, max_len_threshold, min_len_threshold, list_ids, tg_ca, TSD, detection_proba):
    ...
    localresults = [pool.apply_async(sequences_extractor_slave,
                                     args=[splitted_genome_list[x], x, seqs_per_procs, n,
                                           remain,
...
"""
def Inpactor2_Filter_CNN(splitted_genome_ltr, ids_predicted, positions_detected, filter_threshold):
    extracted_sequences = np.zeros(
        (len(positions_detected), splitted_genome_ltr.shape[1], splitted_genome_ltr.shape[2]), dtype=bool)
    new_position_predicted = []
    new_ids_predicted = []
    installation_path = os.path.dirname(os.path.realpath(__file__))

    for i in range(len(positions_detected)):
        init = int(positions_detected[i][0])
        end = int(positions_detected[i][1])
        window_index = int(positions_detected[i][2])
        extracted_sequences[i, :, init:end] = splitted_genome_ltr[window_index, :, init:end]

    # loading DNN model and predict labels (lineages)
    model_path = installation_path + '/Models/Inpactor_Filter_CNN.hdf5'
    model = tf.keras.models.load_model(model_path, custom_objects={'f1_m': f1_m, 'LeakyReLU': tf.keras.layers.LeakyReLU(alpha=0.01)})
    predictions = model.predict(extracted_sequences)
    binary_predictions = [argmax(x) for x in predictions]
    filtered_elements = np.zeros((len([x for x in range(len(binary_predictions)) if binary_predictions[x] == 0 and predictions[x, 0] > filter_threshold]), splitted_genome_ltr.shape[1], splitted_genome_ltr.shape[2]), dtype=bool)

    j = 0  # index of filtered_elements array
    for i in range(len(binary_predictions)):
        if binary_predictions[i] == 0 and predictions[i, 0] > filter_threshold:
            filtered_elements[j, :, :] = extracted_sequences[i, :, :]
...
"""
def Inpactor2_kmer(batch_size, extracted_sequences):
...
def convert_seqs_cycle_master(splitted_genome_ltr, new_pos_predicted, threads):
    n = len(new_pos_predicted)
    seqs_per_procs = int(n / threads)
    remain = n % threads
    pool = multiprocessing.Pool(processes=threads)
    localresults = [pool.apply_async(convert_seqs_cycle_slave,
                                     args=[splitted_genome_ltr, new_pos_predicted, x, seqs_per_procs, n,
                                           remain]) for x in range(threads)]
    localTables = [p.get() for p in localresults]

    # to join local results of extracted sequences
    converted_seqs_all = []
    for i in range(0, len(localTables)):
        converted_seqs_all.extend(localTables[i])
    return converted_seqs_all

def convert_seqs_cycle_slave(splitted_genome_ltr, new_pos_predicted, x, seqs_per_procs, n, remain):
    partial_results = []
    if x < remain:
        i = x * (seqs_per_procs + 1)
        m = n if i + seqs_per_procs + 1 > n else i + seqs_per_procs + 1
    ...
    if i < m:
        ...
        while i < m and i < len(new_pos_predicted):
            init = int(new_pos_predicted[i][0])
            end = int(new_pos_predicted[i][1])
            window_index = int(new_pos_predicted[i][2])
            partial_results.append(one_hot2fasta(splitted_genome_ltr[window_index, :, init:end]))
            i += 1
...
if __name__ == '__main__':
    ...
    for cycle in range(0, cycles):
        ...
        pos_predicted, ids_predicted = sequences_extractor_master(splitted_genome_ltr, threads, outputDir, max_len_threshold, min_len_threshold, list_ids, tg_ca, TSD, detection_proba)
        ...
        if curation:
            ...
            splitted_genome_ltr_filtered, ids_predicted, new_pos_predicted = Inpactor2_Filter_CNN(splitted_genome_ltr, ids_predicted,
            ...
            if splitted_genome_ltr_filtered.shape[0] == 0:
                print(
            ...
            splitted_genome_ltr_filtered = splitted_genome_ltr
        ...
        start = time.time()
        kmer_counts = Inpactor2_kmer(batch_size, splitted_genome_ltr_filtered)
        finish = time.time()
        total_time.append(finish - start)
        ...
        start = time.time()
        predictions, percentages = Inpactor2_Class(kmer_counts)
        finish = time.time()
        total_time.append(finish - start)
        ...
        converted_seqs = convert_seqs_cycle_master(splitted_genome_ltr, new_pos_predicted, threads)
        ltr_predicted_final_cycles.extend(converted_seqs)
