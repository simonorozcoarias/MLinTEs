...
def create_dataset(list_seqs, total_win_len, outputdir, x):
    j = 0
    if len(list_seqs) > 0:
        dataset = np.zeros((len(list_seqs), 5, total_win_len), dtype=np.uint8)
...
def fasta2one_hot(sequence, total_win_len):
    langu = ['A', 'C', 'G', 'T', 'N']
    posNucl = 0
    rep2d = np.zeros((1, 5, total_win_len), dtype=np.uint8)
...
def sequences_extractor(splitted_genome, position_predictions, x, seqs_per_procs, n, remain, outputdir,
                        max_len_threshold, min_len_threshold, list_ids, tg_ca, TSD, detec_threshold):
    if x < remain:
    ...
    if i < m:
            ...
            if isTE > detec_threshold:
                ...
                if len(bestCandidates) > 0:
                    extracted_seqs_i = np.zeros(
                        (len(bestCandidates), splitted_genome.shape[1], splitted_genome.shape[2]), dtype=np.uint8)
                    ...
                    for c in range(len(bestCandidates)):
                        ...
                        predicted_ids.append(seq_id + "#" + str(init + factor) + "#" + str(end + factor) + "#" + str(isTE))
        ...
        if extracted_seqs.shape[1] > 1:  # Process find some LTR-RTs
            np.save(outputdir + '/extracted_seqs_2d_' + str(x) + '.npy', extracted_seqs.astype(np.uint8))
            return np.zeros((10, 10), dtype=np.uint8)
        else:  # Process did not find any LTR-RT
            return np.zeros((1, 1), dtype=np.uint8)
...
def adjust_seq_positions_LTR_Finder(extracted_seq, outputDir, idProc, max_len_threshold, min_len_threshold, tg_ca, TSD):
    seq1file = open(outputDir + '/splittedChrWindow_' + str(idProc) + '.fasta', 'w')
    iterSeq = one_hot2fasta(extracted_seq)
    ...
    try:
        ...
        # print('ltr_finder -F ' + finder_filter + ' -D ' + str(max_len_threshold) + ' -d ' + str(min_len_threshold) + ' -w2 ' + outputDir + '/splittedChrWindow_' + str(idProc) + '.fasta')
        output = subprocess.run(
            ['ltr_finder', '-F', finder_filter, '-D', str(max_len_threshold), '-d', str(min_len_threshold), '-w2',
...
"""
def Inpactor2_Filter(kmer_counts, ids_predicted, predicted_ltr_rts):
    installation_path = os.path.dirname(os.path.realpath(__file__))
    # Scaling
    ...
    filtered_elements = np.zeros((len([x for x in binary_predictions if x == 0]), kmer_counts.shape[1]), dtype=np.uint8)
    new_ltr_predicted = np.zeros((len([x for x in binary_predictions if x == 0]), predicted_ltr_rts.shape[1], predicted_ltr_rts.shape[2]), dtype=np.uint8)
    new_ids_predicted = []
    j = 0  # index of filtered_elements array
    for i in range(len(binary_predictions)):
        if binary_predictions[i] == 0:
            filtered_elements[j, :] = kmer_counts[i, :]
            new_ltr_predicted[j, :, :] = predicted_ltr_rts[i, :, :]
            new_ids_predicted.append(ids_predicted[i] + "#" + str(predictions[i, 0]))
            j += 1
    return filtered_elements, new_ids_predicted, new_ltr_predicted
...
def non_maximal_suppression(ids_predicted, predictions, percentages, iou_threshold, curation, predicted_ltr_rts):
    ...
    for seqid in finalIds:
        columns = seqid.split("#")
        perClass = percentages[i]
        if curation:
            percFilt = float(columns[4])
            elements_dict[seqid] = (percFilt + perClass) / 2
        else:
            elements_dict[seqid] = perClass

    sorted_dict = dict(sorted(elements_dict.items(), key=lambda item: item[1]))
    marked_seqid = []
    deleted_pos = []
    for key in sorted_dict.keys():
        if key not in marked_seqid:
            marked_seqid.append(key)
            percentage = sorted_dict[key]
            pos_seqKey = finalIds.index(key)
            for seqid in sorted_dict.keys():
                if key != seqid:
                    pos_seqId = finalIds.index(seqid)
                    if predictions[pos_seqKey] == predictions[pos_seqId] and iou(key, seqid) > iou_threshold and percentage >= sorted_dict[seqid]:
                        deleted_pos.append(pos_seqId)
                        marked_seqid.append(seqid)

    # to delete elements that are non_maximal
    new_percentages = [percentages[x] for x in range(len(percentages)) if x not in deleted_pos]
    new_predictions = [predictions[x] for x in range(len(predictions)) if x not in deleted_pos]
    finalIds = [finalIds[x] for x in range(len(finalIds)) if x not in deleted_pos]
    new_ltr_predicted = np.delete(predicted_ltr_rts, deleted_pos, 0)
...
def iou(seqX, seqY):
    ...
    intersection = min(endPosX, endPosY) - max(initPosX, initPosY)
    union = max(endPosX, endPosY) - min(initPosX, initPosY)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--tsd', required=False, dest='TSD',
                        help='Keep only elements with TDS? [yes ror not]')
    ...
    splitted_genome = np.zeros((n, 5, total_win_len), dtype=np.uint8)
    ...
    dataset = None  # to clean unusable variable
    localTables = None  # to clean unusable variable
    current, peak = tracemalloc.get_traced_memory()  # to track memory used
    print(
    ...
    localresults = [pool.apply_async(sequences_extractor,
                                     args=[splitted_genome_list[x], position_predictions, x, seqs_per_procs, n, remain,
                                           outputDir, max_len_threshold, min_len_threshold, list_ids, tg_ca, TSD, detec_threshold]) for x
                    in range(threads)]
    localTables = [p.get() for p in localresults]
    ...
    if curation:
        ...
        start = time.time()
        filtered_seqs, ids_predicted, new_ltr_predicted = Inpactor2_Filter(kmer_counts, ids_predicted, ltr_rts_predicted)
        ...
        new_ltr_predicted = ltr_rts_predicted
        kmer_counts = None  # clean unusable variable
    ...
    start = time.time()
    finalIds, predictions, percentages, ltr_predicted_final = non_maximal_suppression(ids_predicted, predictions, percentages, iou_threshold, curation, new_ltr_predicted)
