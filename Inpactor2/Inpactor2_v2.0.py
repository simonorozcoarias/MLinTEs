...
def check_nucleotides_master(list_seqs, threads):
    ...
    for i in range(len(localChecks)):
        if localChecks[i] == 1:
            print(
                "FATAL ERROR: DNA sequences must contain only A, C, G, T, or N characters, please fix it (for example "
                "running the filter utility of Inpactor2_utils.py) and re-run Inpactor2")
            sys.exit(0)
