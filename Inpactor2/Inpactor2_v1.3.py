                        ...
                        if min_len_threshold < int(columns[3]) < max_len_threshold:
                            ...
                            if int(columns[6]) - 5 >= 0 and int(columns[9]) + 5 < extracted_seq.shape[1]:
                                if hamming_distance(extracted_seq[int(columns[6]) - 6:int(columns[6])],
                                                    extracted_seq[int(columns[9]):int(columns[9]) + 6]) <= TSD:
                                    # If TG-CA variable is activated, search if hits starts with TG and ends in CA
                                    if tg_ca:
                                        if iterSeq[int(columns[6]):int(columns[6]) + 2] == 'TG' and iterSeq[int(
                                                columns[7]) - 2:int(columns[7])] == 'CA':
...
if __name__ == '__main__':
    ...
    ltr_rts_predicted = np.load(outputDir + '/extracted_seqs_2d_' + str(0) + '.npy')
    try:
        os.remove(outputDir + '/extracted_seqs_2d_' + str(0) + '.npy')
    except FileNotFoundError:
        print('FATAL ERROR: I could not find: ' + outputDir + '/extracted_seqs_2d_' + str(i) + '.npy')
    for i in range(1, len(localTables)):
    ...
    for i in range(threads):
        ...
        try:
            ...
            print('FATAL ERROR: I could not find: ' + outputDir + '/predicted_ids_' + str(i) + '.txt')
