...
"""
def kmer_counting(file, id, seqs_per_procs, kmers, dicc_splitted_seqs, n, remain, outputDir, w):
	#print("process Inited in "+str(id))
	...
	kmer_table = np.zeros((end-init, len(kmers)), dtype=int)
	index_table = 0
			...
			for l in range(1, 7):
				if i+l < len(TEseq):
					if TEseq[i:i+l].upper().find("N") == -1 and TEseq[i:i+l].upper() in kmers:
						...
						frequencies[index] += 1
		kmer_table[index_table] = frequencies
		init += 1
		index_table += 1
	#print("process finished in "+str(id))
	return kmer_table

"""
This function takes the k-mer table pre-calculated and sums them in windows of lenght l, and write it into a file
"""
	...
	for seqs in list(dicc_seq_sizes):
		...
		for i in range(posIni, posEnd, 1):
			if (i + step) >= posEnd:
				...
				currentEnd = i + step
				stillInSeq = 0
			kmer_count = list(kmer_table[i: currentEnd, :].sum(axis=0))
			frequenciesStrings = [str(x) for x in kmer_count]
...
formart in the ID: seq-name#init
"""
...
def joining_predictions(file, seqIds,  predictions, outputDir, length):
	...
	ids_from_seqs = [seq.id for seq in SeqIO.parse(outputDir+'/'+file, "fasta")]
...
if __name__ == '__main__':
	...
	if kmer_file == None:
		...
		localresults = [pool.apply_async(kmer_counting, args=[file, x, seqs_per_procs, kmers, dicc_splitted_seqs, n, remain, outputDir, window]) for x in range(threads)]
		kmer_tables = [p.get() for p in localresults]
		print("initing to join partial results")
		kmer_table = kmer_tables[0]
		for i in range(1, len(kmer_tables)):
			kmer_table = np.concatenate((kmer_table, kmer_tables[i]), axis=0)
